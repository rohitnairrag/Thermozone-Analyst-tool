...
export const fetchWeather = async (lat: number, lon: number): Promise<HourlyWeather | null> => {
  try {
    ...
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=temperature_2m,relative_humidity_2m,direct_radiation,diffuse_radiation,shortwave_radiation,windspeed_10m,precipitation&timezone=auto&forecast_days=1`;
...
export const fetchWeatherForDate = async (
  ...
  try {
    ...
    const url = `https://archive-api.open-meteo.com/v1/archive?latitude=${lat}&longitude=${lon}` +
      ...
      `&hourly=temperature_2m,relative_humidity_2m,direct_radiation,diffuse_radiation,shortwave_radiation,windspeed_10m,precipitation` +
